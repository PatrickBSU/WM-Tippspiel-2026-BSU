  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-end justify-between mb-6">
        <div>
          ...
          <p className="text-muted text-sm mt-1">
            {betCount} von {totalMatches} Spielen getippt · Auto-Speicherung
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {(grouped[selectedStage] || []).map(m => (
          <MatchCard key={m.id} match={m} initialBet={betMap.get(m.id) || null} />
        ))}
        {(!grouped[selectedStage] || grouped[selectedStage].length === 0) && (
          <div className="text-muted text-center py-12">
            Keine Spiele vorhanden. Admin muss Spielplan importieren.
          </div>
        )}
      </div>
    </div>
