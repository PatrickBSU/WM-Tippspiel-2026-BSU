...
export default async function RanglistePage() {
  ...
  const { data: leaderboard } = await supabase.from("leaderboard").select("*").order("total_points", { ascending: false });
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r, i) => {
                ...
                return (
                  <tr key={r.id} className={`border-b border-border last:border-0 ${isCurrentUser ? "bg-accent/5" : ""}`}>
                    ...
                    <td className="px-4 py-3 font-medium">{r.display_name}{isCurrentUser && <span className="ml-2 text-xs text-accent">(du)</span>}</td>
              ...
              {rows.length === 0 && (<tr><td colSpan={8} className="text-center py-12 text-muted">Noch keine Teilnehmer</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
      <div className="mt-4 text-xs text-muted">Spalten: <strong>Spiele</strong> = Punkte aus 104 WM-Spielen · <strong>Volltreffer</strong> = Anzahl exakt richtiger Ergebnisse · <strong>WM</strong>/<strong>Schütze</strong>/<strong>Gruppen</strong> = Sonderwetten-Punkte</div>
    </div>
  );
}
