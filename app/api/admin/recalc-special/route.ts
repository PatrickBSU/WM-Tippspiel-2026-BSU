...
export async function POST() {
  ...
  const { data: results } = await supabase.from("special_results").select("*").eq("id", 1).single();
  ...
  for (const bet of bets || []) {
    ...
    await supabase.from("special_bets").update({ champion_points: cp, top_scorer_points: tp, group_winners_points: gp }).eq("user_id", bet.user_id);
    updated++;
  }
