...
export default async function AdminPage() {
  ...
  const { data: specialResults } = await supabase.from("special_results").select("*").eq("id", 1).single();
  const { data: matches } = await supabase.from("matches").select("id, stage, kickoff, home_team, away_team, home_score, away_score, status").order("kickoff", { ascending: true });
