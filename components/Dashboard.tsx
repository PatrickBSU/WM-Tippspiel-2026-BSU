...
export default async function Dashboard({ userId }: { userId: string }) {
  ...
  const { data: leaderboard } = await supabase.from("leaderboard").select("id, display_name, total_points, exact_hits").order("total_points", { ascending: false });
  ...
  const { data: nextMatches } = await supabase.from("matches").select("*").gt("kickoff", new Date().toISOString()).order("kickoff", { ascending: true }).limit(5);
  const { data: recentMatches } = await supabase.from("matches").select("*").eq("status", "FINISHED").order("kickoff", { ascending: false }).limit(3);
  ...
  const deadline = process.env.NEXT_PUBLIC_TIPP_DEADLINE ? new Date(process.env.NEXT_PUBLIC_TIPP_DEADLINE) : new Date("2026-06-11T20:00:00+02:00");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      <div>
        ...
        <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tightest">{profile?.display_name || "Tipper"}</h1>
      </div>
      ...
      <section>
        <div className="flex items-baseline justify-between mb-3">
          ...
          <Link href="/tippen" className="text-sm text-accent hover:underline">Alle Spiele →</Link>
        </div>
        <div className="space-y-2">
          {(nextMatches || []).map(m => {
            ...
            return (
              <Link key={m.id} href={`/tippen?stage=${m.stage}`} className="block bg-surface border border-border rounded-lg p-4 hover:border-accent/50 transition-colors">
          ...
          {(!nextMatches || nextMatches.length === 0) && (<div className="text-muted text-center py-8 bg-surface border border-border rounded-lg">Keine Spiele eingeplant. Admin muss Spielplan importieren.</div>)}
        </div>
      </section>
      {recentMatches && recentMatches.length > 0 && (
        <section>
          <div className="flex items-baseline justify-between mb-3">
            ...
            <Link href="/ergebnisse" className="text-sm text-accent hover:underline">Alle Ergebnisse →</Link>
          </div>
          <div className="space-y-2">
            {recentMatches.map(m => (
