...
export default async function ErgebnissePage({ searchParams }: { searchParams: { stage?: string } }) {
  ...
  // Hole alle Spiele die schon angepfiffen haben oder beendet sind
  ...
  if (matchIds.length > 0) {
    const { data } = await supabase
      .from("bets")
      .select("match_id, home_score, away_score, points, user_id, profiles(display_name)")
      .in("match_id", matchIds);
    bets = data || [];
  }

  // Gruppiere Tipps pro Spiel
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-muted text-sm mb-6">
        Spiele mit Anpfiff in der Vergangenheit · Alle Tipps werden öffentlich
      </p>
      ...
      <div className="overflow-x-auto -mx-4 px-4 mb-6">
        <div className="flex gap-1 min-w-max">
          {stages.map(s => (
            <a
              key={s}
              href={`/ergebnisse?stage=${s}`}
              className={`px-3 py-1.5 text-sm rounded-md whitespace-nowrap transition-colors ${
                s === selectedStage ? "bg-accent text-bg font-medium" : "text-muted hover:text-ink hover:bg-surface"
              }`}
            >
              {STAGE_LABELS[s] || s}
            </a>
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-mono text-muted">
                      {new Date(m.kickoff).toLocaleString("de-AT", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                    </span>
                  ...
                  <div className="flex items-center gap-3">
                    {hasResult ? (
                      <span className="font-mono font-bold text-xl">{m.home_score} : {m.away_score}</span>
                    ) : (
                      <span className="text-muted text-sm">läuft / kein Ergebnis</span>
                    )}
